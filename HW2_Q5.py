...
filewriter = tf.summary.FileWriter("finall/20")

sess = tf.Session()
sess.run(tf.global_variables_initializer())
saver = tf.train.Saver()    
tf.summary.FileWriter('finall/20',sess.graph)
...
#nEpoc = np.round(Iter*BatchSize / 55000)
nEpoc = 4
#print(nEpoc)
L_Epoc_test = np.zeros([nEpoc])
L_Epoc_train = np.zeros([nEpoc])

for i in range(nEpoc):
    L_Epoc_test[i] = L_test[1100*(i+1)-1,]
    L_Epoc_train[i] = L_train[1100*(i+1)-1,]
    
plt.figure()
plt.plot(np.arange(1,nEpoc+1),L_Epoc_train)
plt.plot(np.arange(1,nEpoc+1),L_Epoc_test)
plt.title("learningRate: %g"%(lr))
plt.xlabel('Epoc')
plt.ylabel('Loss')
